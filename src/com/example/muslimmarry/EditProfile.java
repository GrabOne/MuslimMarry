...
public class EditProfile extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(user_info.get(prefUser.KEY_AVATAR).toString().length() > 0){
			Picasso.with(getActivity()).load(user_info.get(prefUser.KEY_AVATAR)).fit().centerInside().into(avatar);
			Picasso.with(getActivity()).load(user_info.get(prefUser.KEY_AVATAR)).fit().centerInside().into(large_img);
		}else{
...
//					JSONObject coordinates = new JSONObject(locate.getString("coordinates"));
	...
	public void onClick(View v) {
		...
		case R.id.tvshare:
			((MainActivity)getActivity()).hideKeyboard();
			selectFragCustomize(new SharePhoto());
			break;
